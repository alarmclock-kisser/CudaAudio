...
namespace CudaAudio.Core
{
	public class AudioObj : IDisposable, INotifyPropertyChanged
	{
		...
		{
			int maxWorkers = CommonStatics.OperationalThreads;
			...
			int floatsPerWorker = dataLengthInFloats / maxWorkers;
			var tasks = new List<Task>();

			for (int i = 0; i < maxWorkers; i++)
			{
				int startFloatIndex = i * floatsPerWorker;
				int floatsToRead = (i == maxWorkers - 1) ? dataLengthInFloats - startFloatIndex : floatsPerWorker;
			...
			System.Drawing.Color? graphColor = null, System.Drawing.Color? backgroundColor = null, bool smoothEdges = true)
		{
			...
			await Task.Run(() =>
			{
				...
				int chunkWidth = width / maxWorkers;
				Parallel.For(0, maxWorkers, i =>
				{
					int xStart = i * chunkWidth;
					int xEnd = (i == maxWorkers - 1) ? width : xStart + chunkWidth;
